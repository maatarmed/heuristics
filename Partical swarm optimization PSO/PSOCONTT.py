...
# for rosenbrock i chose [-2.048, 2.048] and for schwefel [-500,500]
...
# limiting the position to exceed one of the boundries
def limiting(position):
	global INF, SUP
	if (position < INF) or (position > SUP):
		position = random.uniform(INF, SUP)
	return position
...
pZoom = 1  # zoom setting
TPSPause = 0.01  # for displaying
...
def eval(sol):
	...
	if FUNCTION == "sphere":
		...
		return schwefel_func(sol)
	elif FUNCTION == "rosenbrock":
		return rosenbrock_func(sol)
	else:
		print("filou, va!")
		exit(1)
...
# schwefel function definition where x = sol
# the length of x represents the dimensions we are working in
def schwefel_func(x):
	f = 0
...
# rosenbrock function definition where x = sol
# the length of x represents the dimensions we are working in
...
for i in range(Nb_cycles):
	# Update informations
	#swarm = [loca(e, best_cycle) for e in swarm]
	swarm = [localUpdate(e, swarm, 10) for e in swarm]
